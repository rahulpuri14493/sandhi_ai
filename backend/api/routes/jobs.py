...
def _validate_allowed_tools(
    ...
    if platform_ids and len(platform_ids):
        ...
        if invalid:
            raise HTTPException(
                ...
                detail=f"Invalid or unauthorized platform tool ids: {sorted(invalid)}",
            )
    ...
    if connection_ids and len(connection_ids):
        valid = (
            db.query(MCPServerConnection.id)
            .filter(
        ...
        if invalid:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Invalid or unauthorized connection ids: {sorted(invalid)}",
            )
...
def _validate_tool_visibility(v: Optional[str]) -> Optional[str]:
    ...
    raise HTTPException(
        status_code=status.HTTP_400_BAD_REQUEST,
        detail="tool_visibility must be 'full', 'names_only', or 'none'",
    )
...
async def create_job(
    ...
    if (platform_ids and len(platform_ids)) or (connection_ids and len(connection_ids)):
        platform_ids, connection_ids = _validate_allowed_tools(
            db, current_user.id, platform_ids, connection_ids
        )
    ...
    new_job = Job(
        ...
        allowed_platform_tool_ids=(
            json.dumps(platform_ids) if platform_ids is not None else None
        ),
        allowed_connection_ids=(
            json.dumps(connection_ids) if connection_ids is not None else None
        ),
        tool_visibility=tv,
    )
    db.add(new_job)
...
async def analyze_documents(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid job files data"
        )
    ...
    documents = [
        {"path": f["path"], "name": f.get("name", "Unknown")}
        for f in files_data
        if f.get("path")
    ]
    if not documents:
            ...
            new_conversation.append(
                {
                    ...
                    "workflow_collaboration_hint": result.get(
                        "workflow_collaboration_hint"
                    ),
                    "workflow_collaboration_reason": result.get(
        ...
        for f in files_data:
            ...
            try:
                ...
                documents_content.append(
                    {"name": name, "content": f"[Could not read {name}]"}
                )
    ...
    for job in jobs:
        ...
        if job.files:
            try:
                ...
                files_data = [
                    {k: v for k, v in f.items() if k != "path"} for f in files_parsed
                ]
            except (json.JSONDecodeError, TypeError):
                pass
...
def auto_split_workflow(
    ...
    preview = workflow_builder.auto_split_workflow(
        job_id,
        body.agent_ids,
...
def update_workflow_step_tools(
    ...
    step = (
        db.query(WorkflowStep)
        .filter(WorkflowStep.id == step_id, WorkflowStep.job_id == job_id)
        .first()
    )
    if not step:
        raise HTTPException(
    ...
    if (
        body.allowed_platform_tool_ids is not None
        or body.allowed_connection_ids is not None
    ):
...
def download_job_file(
    ...
    if not job.files:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="No files found for this job"
        )
    ...
    return FileResponse(
        path=file_path, filename=file_info["name"], media_type=file_info["type"]
    )
