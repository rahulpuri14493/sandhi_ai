...
@router.post(
    "/positions",
    response_model=HiringPositionResponse,
...
def create_hiring_position(
    ...
    new_position = HiringPosition(
        business_id=current_user.id, **position_data.model_dump()
    )
    db.add(new_position)
    db.commit()
...
def list_hiring_positions(
    ...
    for position in positions:
        nomination_count = (
            db.query(func.count(AgentNomination.id))
            .filter(AgentNomination.hiring_position_id == position.id)
            .scalar()
        )
...
def get_hiring_position(
    ...
    if not position:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Hiring position not found"
        )

    # Get nominations
    nominations = (
        db.query(AgentNomination)
        .filter(AgentNomination.hiring_position_id == position_id)
        .all()
    )
...
@router.post(
    "/nominations",
    response_model=AgentNominationResponse,
    status_code=status.HTTP_201_CREATED,
)
def create_nomination(
    ...
    position = (
        db.query(HiringPosition)
        .filter(HiringPosition.id == nomination_data.hiring_position_id)
        .first()
    )
    if not position:
        raise HTTPException(
...
def list_nominations(
    ...
    if current_user.role == UserRole.DEVELOPER:
        ...
        positions = (
            db.query(HiringPosition.id)
            .filter(HiringPosition.business_id == current_user.id)
            .subquery()
        )
        query = query.filter(
            AgentNomination.hiring_position_id.in_(db.query(positions.c.id))
        )
    ...
    for nom in nominations:
        ...
        position = (
            db.query(HiringPosition)
            .filter(HiringPosition.id == nom.hiring_position_id)
            .first()
        )
...
@router.put(
    "/nominations/{nomination_id}/review", response_model=AgentNominationResponse
)
def review_nomination(
    ...
    nomination = (
        db.query(AgentNomination).filter(AgentNomination.id == nomination_id).first()
    )
    if not nomination:
        raise HTTPException(
    ...
    position = (
        db.query(HiringPosition)
        .filter(HiringPosition.id == nomination.hiring_position_id)
        .first()
    )
