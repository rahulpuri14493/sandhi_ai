...
def get_developer_earnings(
    ...
    total_earnings = (
        db.query(func.sum(Earnings.amount))
        .filter(
            and_(
    ...
    earnings_list = (
        db.query(Earnings)
        .filter(Earnings.developer_id == current_user.id)
...
def get_developer_stats(
    ...
    comm_count = (
        ...
        .filter(
            or_(
                AgentCommunication.from_agent_id.in_(
                    db.query(Agent.id).filter(Agent.developer_id == current_user.id)
                ),
                AgentCommunication.to_agent_id.in_(
                    db.query(Agent.id).filter(Agent.developer_id == current_user.id)
                ),
            )
        )
...
def get_business_spending(
    ...
    return {"total_spent": float(total_spent), "job_count": job_count}
