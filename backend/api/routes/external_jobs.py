...
def _get_external_api_key() -> Optional[str]:
    """Get the external API key from the settings."""
    return getattr(settings, "EXTERNAL_API_KEY", "").strip() or None
...
def _build_job_response(job: Job, db: Session) -> dict:
    ...
    try:
        job_files = json.loads(job.files) if job.files else None
        job_conversation = json.loads(job.conversation) if job.conversation else None
