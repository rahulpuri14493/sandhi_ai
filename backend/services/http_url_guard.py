"""
SSRF-oriented URL checks for server-side HTTP (platform REST tool, validate, etc.).
...
def _host_from_netloc(netloc: str) -> str:
    ...
    if host.startswith("["):
        ...
        return host
    if host.count(":") > 1:
        return host  # IPv6 without brackets (unusual in URLs)
    if ":" in host:
...
def check_url_safe_for_server_fetch(url: str, *, purpose: str = "http") -> tuple[bool, str]:
    """
    Return (True, "") if the URL may be fetched by the server, else (False, user-safe reason).
    """
    raw = (url or "").strip()
    if not raw:
...
def safe_url_host_for_logs(url: str, *, max_len: int = 120) -> str:
    """Log hostname only (reduces accidental secret leakage from paths/query)."""
    s = (url or "").strip()
    if not s:
        return ""
    try:
        p = urlparse(s if "://" in s else f"https://{s}")
        h = (p.hostname or "")[:max_len]
        return h
    except Exception:
