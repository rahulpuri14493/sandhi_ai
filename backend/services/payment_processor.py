...
class PaymentProcessor:
    ...
    def calculate_job_cost(self, job_id: int) -> WorkflowPreview:
        ...
        for step in workflow_steps:
            ...
            step_cost = (
                (agent.price_per_task if agent else 0.0)
                if (step.cost is None or step.cost == 0)
                else step.cost
            )
        ...
        for i in range(len(workflow_steps) - 1):
            ...
            from_agent = (
                self.db.query(Agent).filter(Agent.id == from_step.agent_id).first()
            )
    ...
    def process_payment(self, job_id: int) -> Transaction:
        ...
        existing_transaction = (
            self.db.query(Transaction).filter(Transaction.job_id == job_id).first()
        )
    ...
    def distribute_earnings(self, job_id: int):
        ...
        workflow_steps = (
            self.db.query(WorkflowStep).filter(WorkflowStep.job_id == job_id).all()
        )
        ...
        if workflow_step_ids:
            ...
            communications = (
                self.db.query(AgentCommunication)
                .filter(
                    or_(
        ...
        for comm in communications:
            from_agent = (
                self.db.query(Agent).filter(Agent.id == comm.from_agent_id).first()
            )
