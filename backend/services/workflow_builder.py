...
class WorkflowBuilder:
    ...
    def auto_split_workflow(
                ...
                for file_info in files_data:
                    ...
                    if file_path:
                        try:
                            ...
                            logger.debug(
                                "Successfully read document: %s - Content length: %s chars",
                                file_info.get("name"),
                                len(content),
                            )
                        ...
                        logger.warning(
                            "Document %s has no file path", file_info.get("name")
                        )
            except (json.JSONDecodeError, TypeError, Exception) as e:
                # If document parsing fails, continue without document content (documents are optional)
                logger.warning(
                    "Failed to parse job.files: %s - Continuing without documents (they are optional)",
                    e,
                )
        ...
        if documents_content:
            logger.debug(
                "%s document(s) will be included as additional information",
        ...
        if agents:
            splitter = (
                agents[0]
                if (agents[0].api_endpoint and (agents[0].api_endpoint or "").strip())
                else None
            )
            try:
                if splitter:
            ...
            if not task_assignments:
                task_assignments = [
                    {
                        "agent_index": i,
                        "task": f"Execute the job. You are agent {i+1} of {len(agents)}. {job.description or job.title}",
                    }
        ...
        logger.debug(
            "Workflow mode: %s (workflow_mode=%r, BRD hint used when None)",
            "independent" if steps_independent else "sequential",
            workflow_mode,
        )
        ...
        step_ids = [
            s.id
            for s in self.db.query(WorkflowStep.id)
            .filter(WorkflowStep.job_id == job_id)
            .all()
        ]
        if step_ids:
            self.db.query(Earnings).filter(
                Earnings.workflow_step_id.in_(step_ids)
            ).update({Earnings.workflow_step_id: None}, synchronize_session=False)
            comm_filter = or_(
            ...
            comm_ids = [
                r.id
                for r in self.db.query(AgentCommunication.id).filter(comm_filter).all()
            ]
            if comm_ids:
                self.db.query(Earnings).filter(
                    Earnings.communication_id.in_(comm_ids)
                ).update({Earnings.communication_id: None}, synchronize_session=False)
                self.db.query(AgentCommunication).filter(comm_filter).delete(
        ...
        for idx, agent in enumerate(agents):
            ...
            logger.debug(
                "Step %s for agent '%s': depends_on_previous=%s documents=%s conversation_items=%s",
                idx + 1,
                agent.name,
                depends_on_previous,
                len(step_docs),
            ...
            step_visibility = (
                step_tool_visibility
                if step_tool_visibility is not None
                else (tool_visibility or getattr(job, "tool_visibility", None))
            )
            step = WorkflowStep(
                job_id=job_id,
                agent_id=agent.id,
                ...
                allowed_platform_tool_ids=(
                    json.dumps(step_platform) if step_platform else None
                ),
    ...
    def create_manual_workflow(
        self, job_id: int, workflow_steps: List[Dict[str, Any]]
    ) -> WorkflowPreview:
        ...
        # Prepare base input data with job context, Q&A conversation, and documents
        logger.debug(
            "Building manual workflow for job %s title=%s conversation_items=%s documents=%s",
            job_id,
            job.title,
            len(conversation_data) if conversation_data else 0,
            len(documents_content),
        )
        ...
        if not documents_content:
            ...
            total_content_length = sum(
                len(doc.get("content", "")) for doc in documents_content
            )
            logger.debug(
                "Total document content length: %s characters", total_content_length
            )
        ...
        if not conversation_data:
            ...
            logger.debug(
                "Conversation includes %s questions and %s completion messages",
                len(
                    [
                        item
        ...
        for step_data in workflow_steps:
            ...
            step_conv = step_input_data.get("conversation", [])
            logger.debug(
                "Manual workflow step %s for agent '%s': documents=%s conversation_items=%s",
                step_order,
                agent.name,
                len(step_docs),
                len(step_conv),
            ...
            if custom_input_data:
                if isinstance(custom_input_data, dict):
                    ...
                    try:
                        custom_data = (
                            json.loads(custom_input_data)
                            if isinstance(custom_input_data, str)
                            else custom_input_data
                        )
